...
<?php
  ...
  if(!isset($_SESSION['logado']) || $_SESSION['logado'] !== true || $_SESSION['tipo'] === 'Voluntário'){
...
?>
    ...
    $query2 = mysqli_query($conn, "SELECT * FROM Instituicao WHERE email = '$email'");
...
<p>Tipo de Estabelecimento: <?php echo $row2['tipo']?> </p> <!--Inserir o tipo de estabelecimento que foi inserido no registo-->
</div>

<div class="w3-container w3-card-4 w3-light-grey w3-text-blue w3-margin">
<h1>O que nos pode doar?</h1> <!--INSERIR UMA CAIXA DE TEXTO COM O QUE UTILIZADOR ESCREVEU QUANDO SE INSCREVEU-->
<?php echo $row2['doacoes']?>
<h1>Disponibilidade de ajuda: </h1>
<!-- <div class="w3-container w3-light-grey w3-text-black w3-margin">
<a href="disponibilidade_parceiro.php" class="w3-item w3-button">Definir Desponibilidade</a>
</div> -->
...
<h1>Voluntários: </h1>
    ...
    <table class="w3-table-all">
      ...
      <?php  while ($row4 = mysqli_fetch_assoc($query4)):?>
        ...
        <?php $query5 = mysqli_query($conn, "SELECT Participante.email, Participante.nome, Participante.morada, Participante.telefone FROM Participante, Disponibilidade WHERE Participante.email = Disponibilidade.email AND Disponibilidade.distrito = '$distrito' AND Disponibilidade.dia_semana = '$dia_semana' AND Participante.tipo = 'Voluntário' AND '$hora_fim' > Disponibilidade.hora_inicio AND '$hora_inicio' < Disponibilidade.hora_fim");?>
